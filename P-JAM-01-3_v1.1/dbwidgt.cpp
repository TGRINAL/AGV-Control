...
void DbWidgt::initModel()
{
    ...
    //qDebug() << model->tableName() << model->columnCount();
...
void DbWidgt::readSettings()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        //qDebug() << "read" << m_name << i << number << identifier;
    }
    settings.endArray();
    settings.endGroup();
    getModel()->submitAll();
...
void DbWidgt::on_addButton_clicked()
{
    ...
    //connect(dlg, &EditDbDialog::addDbFinish, this, &DbWidgt::addDbFinish);
...
void DbWidgt::on_editBtn_clicked()
{
    ...
    if(ui->acTableView->currentIndex().column() < 3){
        EditDbDialog *dlg
                = new EditDbDialog(false, ui->acTableView->currentIndex().model()->index(currentRow, 1).data().toString(),
...
void DbWidgt::addDbFinish(QString numberStr, QString idStr)
{
    ...
    query.exec();
    //qDebug() << lastID.toInt()+1 << numberStr << idStr;
